...
function jpegToTest($name, Jpeg $jpeg)
{
    $exif = $jpeg->getExif();
...
class %s extends ExifEyeTestCaseBase
{
  ...
  function testRead()
  {
    $jpeg = new Jpeg(dirname(__FILE__) . \'/%s\');
', $test_name, $image_filename, $image_filename);

require_once(dirname(__FILE__) . '/../../src/Jpeg.php');
...
if (count($exceptions) == 0) {
    println('$this->assertTrue(count(ExifEye::getExceptions()) == 0);');
} else {
    println('$exceptions = ExifEye::getExceptions();');
